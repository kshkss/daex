...
def constraint(params: Params, t: jax.Array, stat: State) -> jax.Array:
    return stat.x**2 + stat.y**2 - RADIUS**2
...
def analytic(params: Params, ts: jax.Array, stat0: State) -> State:
    def result(params: Params, t: jax.Array, t0: jax.Array, stat0: State):
        ...
        # matching how _run_forward_jvp discards the tangent of the
        # algebraic initial condition.
        phase0 = jnp.arccos(stat0.y / RADIUS)
        phase = params.a * (t - t0) + phase0
        return State(x=RADIUS * jnp.sin(phase), y=RADIUS * jnp.cos(phase))
...
def y0():
    return State(
        y=RADIUS * jnp.cos(PHASE0),
        x=RADIUS * jnp.sin(PHASE0),
    )
...
def test_forward_over_forward_matches_analytic(dae, params, ts, y0, tangent_index):
    ...
    # Second-order forward-over-forward compounds IDA's own solver
    # tolerance twice, so it needs a looser tolerance than the first-order
    # comparison above.
    assert jnp.allclose(second_order, second_order_acc, 1e-3, 1e-3)
