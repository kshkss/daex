...
def y0(ts):
    y_val = jnp.array(1.5)
    return State(y=y_val, x=ts[0] * y_val)
...
def test_denominator_stays_bounded_away_from_zero(params, ts, y0):
    denom = 1.0 + params.a * y0.y * (ts - ts[0])
    assert jnp.all(jnp.abs(denom) > 0.5)
